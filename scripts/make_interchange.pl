...
$drive = "/Users/davidbaxter/DATA/";
$CDL_drive = "/Users/davidbaxter/DATA/cch_scripts";
$ICPN_drive = "/Users/davidbaxter/DATA/Interchange";
$old_dir = $drive."Interchange/_input/";
$rootdir = "/CCH-master/Interchange/_input/";

$outdir = $rootdir."output/";
$logdir = $rootdir."log/";

$tempdir = $old_dir."temp/";
$linksdir = $old_dir."links/";

#open (LOGERR,">$logdir/errors") || die;
$file_smasch_specimen_id = "$CDL_drive/CDL_main.in";  #taxon IDs for which CCH has specimen
$file_smasch_taxon_id = "/Users/davidbaxter/DATA/smasch_taxon_ids.txt";
#$file_smasch_taxon_id = "Smasch_tid.txt";  #SMASCH IDs for which SMASCH has specimen
$file_add_updates = "$ICPN_drive/add_updates.txt"; #Memo of pending changes
$file_cpn = "$ICPN_drive/CPN_out.txt"; #Rosatti's converted Interchange file
$file_current_names = "$ICPN_drive/current_names.txt"; #Synonyms accumulated; maintained by hand, update this before running this script
$file_corrections = "$ICPN_drive/I_XW.txt"; #Mapping of name variants from other floras; maintained by hand
$file_usda_variant = "$ICPN_drive/usda_variant.txt"; #From USDA PLANTS database; Add CRs
#$file_treat = "new_markup.txt"; #Jepson Manual treatments from Jeff Greenhouse
$file_interchange_template = "$ICPN_drive/interchange_templ.txt"; #HTML for Interchange front page [should be pulled off website??]
#$hashfile_taxon_no_author_name = "TNOAN_DB.hash"; #Generated from SMASCH; maps SMASCH no-author names to SMASCH IDs 
$file_cnps_taxon_id = "$ICPN_drive/CNPS_taxon_ids.txt"; #CNPS links; generated by script [name getCNPS???] from CNPS website, file outdated, needs created.. not sure what this if for..

#Output files (other output file names are generated by script)
$file_JM_parseq = $outdir."IJM_sequence.txt"; #Paragraph sequence index, no longer to be appended to CGI script: get_JM_treatment.pl
...
$file_log_hcodes = $tempdir."tid_hcodes.txt"; #List of taxon SMASCH IDs and coloring scheme for bioregion map
...
$hashfile_BID_AIDb = $tempdir."BID_AID.hash"; #type information hash file, not updates since 2012, no update files, added here to add new type info from Interchange, no other herbaria info added except if you use alter_bid_aid.pl
$hashfile_capn_db = $tempdir."capn_db.hash"; #Taxon SMASCH IDs to Interchange contents from $file_cpn_out
$file_TJM1treat_index = $tempdir."TJM_sequence.txt"; #Indices of treatment paragraphs for TJM1 CGI script (old file do not change)
$file_JMtreat_index = $tempdir."IJM_sequence_names.txt"; #Indices of treatment paragraphs for CGI script
...
open(IN, "$file_smasch_taxon_id") || die;
while(<IN>){
	chomp;
...
##############ILDIS was way out of date, so we've commented it out for now
...
#	open(IN,"$JMtreat_index") || die;
	&open_input_file($file_TJM1treat_index);
	warn "opening treatment index $file_TJM1treat_index;\n";
	while(<IN>){
		next if /^$/;
		chomp;
		($name,$pars)=split(/ => /,$_);
...
## number in Jepson Manual Markup.
	...
	#close(OUT);
#########################################################################
unlink($hashfile_tid_par) || warn $!;  #Commented out 1 Sep 2005
...
'11b' => 'taxonomic or nomenclatural synonym for taxonomically not recognized and/or sterile hybrid (hybrid form of name)',
'14_t' => 'orthographic variant of name treated elsewhere in this list (tentative)',
'unresolved' => 'Current Status not yet established',
'pending_1' => 'accepted name for taxon native to CA',
'pending_1a' => 'taxonomic or nomenclatural synonym for taxon native to CA',
'pending_2' => 'accepted name for taxon naturalized in CA',
'pending_8' =>'accepted name for taxon not occurring in CA (erroneous reports, misapplication of names, misidentifications, other exclusions)'
	);

unlink($hashfile_capn_db) || die "$! $hashfile_capn_db\n";  #Commented out 1 Sep 2005
	...
	tie %BID_AID, "BerkeleyDB::Hash", -Filename => $hashfile_BID_AID, -Flags    => DB_CREATE or die "Cannot open file $filename: $! $BerkeleyDB::Error\n" ;
	print "here: $hashfile_BID_AID\n";
	%NAME_CODE=();
	%CAPN=();
	%BID_AID=();
...
#print "$.: skipping done: $_\n";
		...
		#print "$_\n";
			...
			s/^Current Status: *(pending_\d+[a-z]*).*/CS: $1/ ||
...
#}
		...
		#print "$taxon_id\t$x_ref\n";
			...
			$par_number=$TID_TO_PAR{$taxon_id};
			$par_number=~s/.*,// if $par_number;
			if(($type_ref)=grep(/TI: (.+)/,@lines)){
				if($par_number){
					$type_ref=&strip_name($x_ref);
					$TYPE{$par_number}="$type_ref\tt";
					print OUT "NEW TYPE INFO: $par_number: $type_ref\n";
				}
				else{
					print OUT "No TYPE par number: $type_ref\n";
				}
			}
		}
...
<head>
<title>
UC/JEPS: Interchange Index Page for The Jepson Manual eFlora
</title>
$keywords
</head>
<body>
$page_top<br>&nbsp;
<H2 align="center">
Index to Taxa in The Jepson Manual eFlora
</H2>
<UL>
EOP
...
tie(%IJM, "BerkeleyDB::Hash", -Filename=>"/Users/davidbaxter/DATA/eFlora/outputs/IJM.hash")|| die "Stopped; couldnt open IJM\n";
	foreach(keys(%IJM)){
		$key=$_;
...
GO TO OUTPUT. MAKE TAR FILE BY RUNNING tar_interchange.sh.
Upload tar file (interchange.tar) to Darwin into  ~/make_interchange/undump
run undump_interchange.pl
run install.sh (i.e., sh install.sh)

     most important, add 
     "LN2C.txt" to cgi-bin/ln2c.pl on darwin  
     and I_common.txt to get_cn2.pl.
...
open(OUT,">/Users/davidbaxter/DATA/Interchange/_input/log/needed_names.tmp") || die;
print OUT join("\n",@needed_names);
close OUT;
...
EOP
	...
	foreach (keys(%Current_Status)){
		print "$_: $Current_Status{$_}\n";
	}
}
